...
//这是消息回调函数，它处理各种窗口消息，如关闭、键按下、键释放、鼠标按下、鼠标释放和鼠标滚轮事件。
//它相应地更新窗口和输入设备的状态
...
*/
//此函数使用 WNDCLASS 结构注册窗口类。
//它设置窗口类的属性，如样式、窗口过程、实例句柄、图标、光标、背景、菜单和类名。
static void register_window_class()
{
	...
	wc.lpszClassName = "LQC_Render_window";					//该窗口类的名字
...
//此函数初始化 BITMAPINFOHEADER 结构，该结构描述用于显示图形的位图的格式。
//它设置宽度、高度、位数、压缩、图像大小和其他参数。
...
//它注册窗口类，创建具有指定尺寸和标题的窗口，创建设备上下文（DC），
//为双缓冲创建兼容位图，并设置用于显示窗口的必要属性。
int window_init(int width, int height, const char* title)
{
	...
	//创建窗口
	window->h_window = CreateWindow(("LQC_Render_window"), title,
...
//
//此函数执行实际的图形显示。它设置绘图所需的字体、文本颜色和背景颜色，
//并使用 TextOut 和 BitBlt 函数在内存设备上下文上绘制图形，
//并将其传输到实际窗口设备上下文中。
...
//此函数使用提供的帧缓冲数据更新窗口帧缓冲。
//它将输入帧缓冲区的 RGB 值复制到窗口帧缓冲区，
//并调用 window_display() 来显示更新的图形。
